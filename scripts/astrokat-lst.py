...
def main(args):
    ...
    if args.target:
        ...
        return_str = "Target ({}) rises at LST {} and sets" \
                     "at LST {}".format(" ".join(args.target), rise_lst, set_lst)

    elif args.utc and not args.lst:
        utc_datetime = datetime.strptime(args.utc, "%Y-%m-%d %H:%M")
        observer.date = ephem.Date(utc_datetime)
        return_str = "At {}Z MeerKAT LST will be {}".format(observer.date,
        ...
        return_str = "{} {} LST corresponds to {}Z UTC".format(date_str,
                                                               args.lst,
                                                               date_lst
                                                               )
        ...
        return_str += "Now is {}Z UTC and {} LST".format(observer.date,
                                                         observer.sidereal_time()
                                                         )
