...
def _nd_log_msg_(ant,
    ...
    msg = (
        "Pattern set as {} sec ON for"
        "{} sec cycle length".format(actual_on_frac * actual_cycle, actual_cycle))
    user_logger.info(msg)
...
def on(kat,
    ...
    user_logger.trace("TRACE: nd on at {} ({})".format(timestamp, time.ctime(timestamp)))
...
def pattern(kat,  # kat subarray object
    ...
    if not kat.dry_run:
        ...
        user_logger.warning(
            "Correlator integration time {} [sec]".format(1. / dump_period))
